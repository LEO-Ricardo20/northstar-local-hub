...
- 导航轨与侧栏补齐聚合能力：日志中心弹层（`Ctrl+J` 呼出）、设置中心弹层（任务完成通知开关、浅色/深色/自动外观、版本与目录信息）、快捷操作部件的批量停止服务（确认后逐个安全停止，绝不按端口结束进程）。
- 服务表格新增**进程溯源**：沿 PPID 链识别并显示每个服务的启动者（AI 编程助手/编辑器/终端/LeoDock），副标题行展示来源图标与名称。
- 新增「LeoDock Glass」为唯一 UI 主题（深空蓝黑、雾白玻璃、蓝金光影与分级透明材质），主题清单中固定排首位；保留 `#themeCss` 整包加载与 `uiTheme` 配置机制，但不再提供多主题与主题选择界面。
...
- 增加统一品牌标识、网页 favicon、Windows 应用图标与可重建的品牌导出脚本。
- 侧边栏新增“版本信息”和“使用说明”板块，并同步加入命令面板与键盘可访问的玻璃浮层。
...
- 界面进阶重构为“LeoDock Glass”：以深黑蓝空间、雾白玻璃和 LEO 蓝折射光为核心，建立顶栏、导航、KPI、业务卡片、表格与浮层的分级透明材质，并提供浅色、深色、窄屏、减少动态和高对比度降级。
- 产品与仓库品牌统一为 `LEODOCK / LeoDock`，中文定位为“LEO 本地工作台”，英文定位为“Windows Local Service Workspace”，作者署名统一为 `by LEO-Ricardo20`。
- README 的启动台与服务监控截图更新为“LeoDock Glass”实机界面，并使用合成目录、端口和进程数据避免暴露本机信息。
- README 收敛为LeoDock的独立产品与版权表述，并继续完整保留第三方字体、图标和素材许可。
- 默认将配置/图标移至 `%LOCALAPPDATA%\\LeoDock`，日志移至 `%LOCALAPPDATA%\\LeoDock\\logs`。新目标不存在时仅首次复制旧版 `%LOCALAPPDATA%\\总控台`，不删除原文件。
- `config.json.bak` 现保留修改前的上一份良好配置，而不是与主配置相同的副本。
- 运行目录和文件继承当前 Windows 用户的 ACL，不授予额外共享访问权限。
...
- 修复刷新进程/端口状态及打开原生路径选择器时可能弹出额外 PowerShell 黑窗的问题；Windows 平台命令现在始终以无控制台窗口方式运行。
- 修复后台启动时选择工作目录/脚本无响应或请求超时的问题；选择器现在带有前台所有者、专用交互超时，并防止重复打开。
...
- 移除 Apollo/Candy/8-Bit 三套 UI 主题与主题选择面板、命令面板主题切换项及 Candy 专用 hero 卡；产品收敛为单一「LeoDock Glass」主题，旧的 `uiTheme` 偏好自动迁移到 `leodock-glass`。
...
- 基于随机运行令牌、根 PID、父子进程关系和当前用户 SID 的受控进程识别。
- 原子配置写入、同步备份、有界日志读取与轮转。
- Apollo/Candy 双 UI 主题、深浅色模式、命令面板和原生文件选择。
- Windows 后台启动器与调试入口。
