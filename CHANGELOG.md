...
- 服务表格新增**进程溯源**：沿 PPID 链识别并显示每个服务的启动者（AI 编程助手/编辑器/终端/北辰本地中枢），副标题行展示来源图标与名称。
...
- 界面进阶重构为“北辰光幕”：以深黑蓝空间、雾白玻璃和北辰蓝折射光为核心，建立顶栏、导航、KPI、业务卡片、表格与浮层的分级透明材质，并提供浅色、深色、窄屏、减少动态和高对比度降级。
- README 的启动台与服务监控截图更新为“北辰光幕”实机界面，并使用合成目录、端口和进程数据避免暴露本机信息。
- README 收敛为北辰本地中枢的独立产品与版权表述，并继续完整保留第三方字体、图标和素材许可。
- 默认将配置/图标移至 `%LOCALAPPDATA%\\北辰本地中枢`，日志移至 `%LOCALAPPDATA%\\北辰本地中枢\\logs`。新目标不存在时仅首次复制旧版 `%LOCALAPPDATA%\\总控台`，不删除原文件。
