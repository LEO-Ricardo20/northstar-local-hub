...
感谢你帮助改进LeoDock。项目仍处于 Preview / Alpha 阶段，优先接受范围清晰、可验证且不扩大安全边界的改动。
...
4. 不要提交本机运行数据、`%LOCALAPPDATA%` 内容、个人路径、完整命令、token、用户图标或未脱敏截图。

## 开发环境

- Windows 11 22H2 或更高版本；
- Python 3.12 或更高版本（建议保留 `py.exe` 启动器）；
...
- 不得削弱回环绑定、当前用户 SID、随机运行令牌、根 PID/进程树、Host/Origin 或控制令牌等安全校验。
- 不得按端口直接结束未知进程。
