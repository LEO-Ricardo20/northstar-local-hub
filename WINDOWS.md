...
北辰本地中枢是仅面向 Windows 11 的本地网页应用。后端只绑定 `127.0.0.1`，运行时无需第三方 Python 包，也不会安装 Windows 服务、计划任务或开机启动项。
...
日常使用双击 `start-windows.cmd`。它会在后台启动并打开浏览器；重复双击会识别已有实例。

排查启动错误时双击 `start-windows-debug.cmd`，或运行：
...
旧版“总控台”的数据位于 `%LOCALAPPDATA%\总控台`。首次启动北辰本地中枢时，
...
停止服务时，北辰本地中枢先通过随机运行令牌、根 PID、父子进程链和当前用户 SID 验证身份，再使用 `taskkill /T /F` 回收受控进程树。它不会按端口结束未知进程。
...
2. 停止北辰本地中枢。
3. 删除项目文件夹。
4. 确认不再需要配置后，删除 `%LOCALAPPDATA%\北辰本地中枢`。
