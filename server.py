...
# 公开平台常量供诊断与测试契约使用；运行时实现仅保留 Windows 11。
IS_WINDOWS = True
import ctypes
import msvcrt
...
SELF_PID = os.getpid()
# Windows 没有 POSIX uid。平台层把当前登录用户映射为 1，其他用户为 0，
...
def acquire_instance_lock(path=INSTANCE_LOCK_PATH):
    ...
    lock ties exclusivity to this data directory and is released on crash.
    """
...
def run_powershell(script, timeout=SUBPROCESS_TIMEOUT):
    ...
    executable = (shutil.which("powershell.exe") or
                  ...
                  shutil.which("pwsh"))
    if not executable:
        LOG.error("未找到 PowerShell，无法读取 Windows 进程信息")
        return ""
    preamble = (
    ...
    try:
        ...
        LOG.exception("PowerShell 平台命令执行失败")
        return ""


def _powershell_json(script, timeout=SUBPROCESS_TIMEOUT):
    """读取 PowerShell ConvertTo-Json 输出，单对象也统一为列表。"""
    raw = run_powershell(script, timeout=timeout).lstrip("\ufeff").strip()
    if not raw:
        return []
    try:
        value = json.loads(raw)
    except (json.JSONDecodeError, TypeError):
        LOG.warning("Windows 平台命令返回了无效 JSON")
        return []
    return value if isinstance(value, list) else [value]


def _windows_process_sid(pid):
    """Return a process owner SID string without requiring elevation."""
    ...
    try:
        ...
        if not advapi32.GetTokenInformation(
                token, TOKEN_USER, buffer, needed,
                ctypes.byref(needed)):
            return None
...
WINDOWS_CURRENT_USER_SID = _windows_process_sid(SELF_PID)


def process_owned_by_current_user(pid):
    sid = _windows_process_sid(pid)
    return bool(sid and WINDOWS_CURRENT_USER_SID and
                sid == WINDOWS_CURRENT_USER_SID)


def scan_listeners():
    """Windows TCP 监听快照 → {(pid, port): {bind_host, ...}}。

    字典仍可像旧集合一样迭代/判断 ``(pid, port)``，同时保留监听地址，
    供前端区分仅监听 ``::1`` 的服务（需通过 localhost 打开）。
    """
    now = time.monotonic()
    with WINDOWS_SNAPSHOT_LOCK:
        if now - WINDOWS_LISTENER_CACHE["mono"] < 1.0:
            return {
                key: set(value)
                for key, value in WINDOWS_LISTENER_CACHE["data"].items()
            }
    rows = _powershell_json(
        "@(Get-NetTCPConnection -State Listen | "
        "Select-Object OwningProcess,LocalPort,LocalAddress) | "
    ...
    for row in rows:
        try:
            ...
            continue
        host = str(row.get("LocalAddress") or "")
        found.setdefault((pid, port), set()).add(host)
    with WINDOWS_SNAPSHOT_LOCK:
        WINDOWS_LISTENER_CACHE["mono"] = time.monotonic()
        WINDOWS_LISTENER_CACHE["data"] = {
            key: set(value) for key, value in found.items()
        }
    return found
...
def ps_snapshot(pids=None, with_uid=True):
    """通过 CIM 批量读取 Windows 进程信息。"""
    requested = None if pids is None else {
        int(pid) for pid in pids if isinstance(pid, (int, str))}
    if requested is not None and not requested:
        return {}
    now = time.monotonic()
    cached_result = None
    with WINDOWS_SNAPSHOT_LOCK:
        cached = WINDOWS_PROCESS_CACHE["data"]
        cache_fresh = now - WINDOWS_PROCESS_CACHE["mono"] < 1.0
        if cache_fresh and (requested is None or requested.issubset(cached)):
            selected = cached if requested is None else {
                pid: cached[pid] for pid in requested if pid in cached}
            cached_result = {pid: dict(info)
                             for pid, info in selected.items()}
    if cached_result is not None:
        for pid, info in cached_result.items():
            if with_uid and "uid" not in info:
                info["uid"] = (
                    SELF_UID if process_owned_by_current_user(pid) else 0)
            elif not with_uid:
                info.pop("uid", None)
    ...
    if requested is None:
        ...
        clauses = " -or ".join(
            "$_.ProcessId -eq %d" % pid for pid in sorted(requested))
        query = "Get-CimInstance Win32_Process | Where-Object {%s}" % clauses
    rows = _powershell_json(
        "@(%s | Select-Object " % query +
        "ProcessId,ParentProcessId,Name,ExecutablePath,CommandLine,"
        "CreationDate,WorkingSetSize,KernelModeTime,UserModeTime) | "
        "ConvertTo-Json -Compress",
        timeout=max(SUBPROCESS_TIMEOUT, 12))
    cpu_by_pid = {}
    current_counters = {}
    ...
    for row in rows:
        try:
            ...
            current_counters[pid] = (
                int(row.get("KernelModeTime") or 0) +
                int(row.get("UserModeTime") or 0))
        except (AttributeError, TypeError, ValueError):
            continue
    if requested is None:
        with WINDOWS_SNAPSHOT_LOCK:
            previous_at = WINDOWS_CPU_CACHE["mono"]
            previous = WINDOWS_CPU_CACHE["counters"]
            elapsed = sampled_at - previous_at
            if elapsed > 0 and previous:
                processors = max(1, os.cpu_count() or 1)
                for pid, counter in current_counters.items():
                    old = previous.get(pid)
                    if old is not None and counter >= old:
                        cpu_by_pid[pid] = min(
                            100.0,
                            (counter - old) / 10_000_000.0 /
                            elapsed / processors * 100.0)
            WINDOWS_CPU_CACHE["mono"] = sampled_at
            WINDOWS_CPU_CACHE["counters"] = current_counters
    total_memory = 0
    try:
    ...
    for row in rows:
        ...
        entry = {
            ...
            "etime": max(0, int(now - started)) if started else 0,
            "cpu": cpu_by_pid.get(pid, 0.0),
            "mem": (working_set * 100.0 / total_memory
                    if total_memory else 0.0),
            "comm": str(row.get("ExecutablePath") or row.get("Name") or ""),
            "ppid": int(row.get("ParentProcessId") or 0),
        }
        if with_uid:
            entry["uid"] = SELF_UID if process_owned_by_current_user(pid) else 0
        snap[pid] = entry
    if requested is None:
        with WINDOWS_SNAPSHOT_LOCK:
            WINDOWS_PROCESS_CACHE["mono"] = time.monotonic()
            WINDOWS_PROCESS_CACHE["data"] = {
                pid: dict(info) for pid, info in snap.items()
            }
    return snap


def process_cwds(pids):
    """从 Windows 进程命令行尽力推断工作目录。"""
    ...
    result = {}
    drive_path = re.compile(r'(?i)([A-Z]:\\[^"\r\n]+)')
    for pid, info in snap.items():
        command = info.get("args") or ""
        match = drive_path.search(command)
        if match:
            candidate = match.group(1).rstrip('" ')
            if os.path.isfile(candidate):
                result[pid] = os.path.dirname(candidate)
...
SYSTEM_PROCESS_NAMES = {
    ...
    "svchost.exe", "fontdrvhost.exe", "dwm.exe", "audiodg.exe",
}
...
def classify_group(key, name, comm, args, cwd, promoted):
    ...
    if any(k in text for k in DEV_KEYWORDS):
        return "mine"
    if os.path.basename(comm or name).casefold() in SYSTEM_PROCESS_NAMES:
        return "background"
    return "mine"
...
def origin_snapshot():
    """把 CIM 进程快照转换为 ``pid -> (ppid, args)`` 来源表。"""
    return {
        pid: (info.get("ppid", 0), info.get("args") or info.get("comm") or "")
...
def attribute_origin(pid, table):
    ...
    for _ in range(12):
        ...
        executable_text = parent_args.strip().strip('"')
        executable_text = executable_text.replace("/", "\\").rsplit("\\", 1)[-1]
        executable_parts = executable_text.split('"', 1)[0].split(None, 1)
        executable = executable_parts[0].casefold() if executable_parts else ""
        if executable in _ORIGIN_WINDOWS_APPS:
            label, icon = _ORIGIN_WINDOWS_APPS[executable]
...
def build_watched(keywords):
    ...
    for pid, info in sorted(snap.items()):
        ...
        if name.casefold() in {"powershell.exe", "pwsh.exe", "powershell", "pwsh"}:
            continue
...
def pgid_members_map():
    """根据 Windows 父进程关系构建 ``根 PID -> 进程树成员`` 映射。"""
    snap = ps_snapshot(None, with_uid=False)
    groups = {}
    children = {}
    for child, info in snap.items():
        children.setdefault(int(info.get("ppid") or 0), []).append(child)
    for root in snap:
        members, pending, seen = [], [root], set()
        while pending:
            pid = pending.pop()
            if pid in seen:
                continue
...
def managed_process_index(apps, groups=None):
    ...
    for app in apps:
        ...
        if not controller_found and token:
            root = app.get("lastPgid") or app.get("lastPid")
            # cmd 可能在启动子进程后自然退出；只要记录的根 PID 已消失、
            # 仍有同一父链的当前用户后代，就保留受控身份。随机 token 在
            # 根存活时强校验，避免 PID 复用造成错误认领。
            controller_found = bool(
                isinstance(root, int) and root > 0 and
                root not in snap and current_user)
...
# ---------------------------------------------------------------- 状态快照缓存
# 每次快照要读取端口和 CIM 进程信息。TTL 略大于前端 2s 轮询周期：
...
def invalidate_state_cache():
    ...
    with WINDOWS_SNAPSHOT_LOCK:
        WINDOWS_PROCESS_CACHE["mono"] = 0.0
...
def build_health(cfg):
    ...
    for label, path in (("config", CONFIG_PATH),
        ...
        if not stat.S_ISREG(mode):
            issues.append("%s 文件不是普通文件" % label)
...
def process_uid(pid):
    """将 Windows 进程 SID 所有权映射到既有 uid 契约。"""
    if not pid_alive(pid):
        return None
    return SELF_UID if process_owned_by_current_user(pid) else 0
...
def kill_process(pid, force):
    ...
    if completed.returncode and pid_alive(pid):
        ...
        return False, "结束失败: %s" % message
    invalidate_state_cache()
    return True, None


def stop_pid_tree(pid, sig=signal.SIGTERM):
    """用 ``taskkill /T /F`` 回收已验证的 Windows 受控进程树。"""
    # 隐藏窗口的 cmd/Node/Python 没有可投递 CTRL_C_EVENT 的共享控制台；
    # taskkill 不带 /F 会拒绝结束仍有子进程的根 cmd。仅在 token + 根 PID
    # 已验证后，对整棵受控树做强制回收，绝不按端口选择目标。
    command = ["taskkill.exe", "/PID", str(int(pid)), "/T", "/F"]
    try:
        completed = subprocess.run(
    ...
    if completed.returncode and pid_alive(pid):
        message = (completed.stderr or completed.stdout or "未知错误").strip()
        return False, "停止受控进程树失败: %s" % message
    invalidate_state_cache()
    return True, None
...
def build_launch_env(token, environ=None):
    """构建从 Windows 启动器运行时可找到常见开发工具的环境。"""
    env = dict(os.environ if environ is None else environ)
    home = os.path.expanduser("~")
    app_data = env.get("APPDATA") or os.path.join(
        home, "AppData", "Roaming")
    local_data = env.get("LOCALAPPDATA") or os.path.join(
        home, "AppData", "Local")
    preferred = [
        os.path.join(app_data, "npm"),
        os.path.join(local_data, "pnpm"),
        os.path.join(local_data, "Microsoft", "WindowsApps"),
    ...
    env.setdefault("PNPM_HOME", os.path.join(local_data, "pnpm"))
...
def start_app(app):
    ...
    shell = (os.environ.get("COMSPEC") or
             os.path.join(os.environ.get("SystemRoot", r"C:\Windows"),
                          "System32", "cmd.exe"))
    wrapper_dir = os.path.join(LOGS_DIR, ".run")
    _ensure_private_dir(wrapper_dir)
    wrapper_path = os.path.join(wrapper_dir, "%s-%s.cmd" % (
        app["id"], token[:12]))
    wrapper = (
        "@echo off\r\n"
        "chcp 65001 >nul\r\n"
        "%s\r\n"
        "exit /b %%errorlevel%%\r\n" % app["command"])
    write_private_bytes(wrapper_path, wrapper.encode("utf-8"))
    # marker 作为无语义参数保留在根 cmd 的命令行；实际用户命令写入
    # 临时批处理文件，避免 cmd /C 对引号和 Python -c 内容二次拆解。
    popen_args = [shell, "/D", "/S", "/C", "call", wrapper_path, marker]
    popen_kwargs = {
    ...
    try:
        ...
        proc = subprocess.Popen(
            popen_args, cwd=cwd, stdout=logf, stderr=subprocess.STDOUT,
        ...
        def _cleanup_wrapper():
            try:
                ...
                try:
                    ...
                    pass
        threading.Thread(target=_cleanup_wrapper, daemon=True).start()
    invalidate_state_cache()
...
def command_for_script(path):
    ...
    if suffix == ".py":
        return subprocess.list2cmdline([sys.executable, "--", normalized])
    if suffix == ".ps1":
        return "powershell.exe -NoProfile -ExecutionPolicy Bypass -File %s" % quoted
    if suffix in (".cmd", ".bat"):
        return "%s /D /S /C %s" % (
            subprocess.list2cmdline([os.environ.get("COMSPEC", "cmd.exe")]),
            quoted)
    if suffix == ".js":
        return "node %s" % quoted
    return quoted


SCRIPT_SUFFIXES = {".py", ".ps1", ".cmd", ".bat", ".js"}
SHELL_BUILTINS = {
...
def _simple_command_tokens(command):
    ...
    if not isinstance(command, str) or not command.strip():
        return []
    # Windows 命令允许盘符与反斜杠；复杂 shell 元字符仍保持 unknown，
    # 健康检查绝不执行或展开用户输入。
...
def _script_target(tokens, cwd):
    ...
    if index >= len(tokens):
        return None, False, False
    executable = tokens[index].strip('"')
    base = os.path.basename(executable)
    args = [arg.strip('"') for arg in tokens[index + 1:]]

    if (re.fullmatch(r"python(?:\.exe|\d+(?:\.\d+)*)?", base, re.I)
            or base.casefold() in {"py", "py.exe"}):
    ...
    if base.casefold() in {"cmd", "cmd.exe"}:
        lowered = [arg.casefold() for arg in args]
        candidate = args[-1] if args else None
        if candidate and os.path.splitext(candidate)[1].lower() in {".cmd", ".bat"}:
            return (_resolve_command_path(candidate, cwd), False,
                    not os.path.isabs(os.path.expanduser(candidate)))
        return None, False, False
...
def inspect_app_health(app):
    ...
    if executable and not direct and executable_base not in SHELL_BUILTINS:
        if os.path.isabs(executable.strip('"')) or "/" in executable or "\\" in executable:
...
def detect_project(root):
    ...
    if package_text is not None:
        ...
        for index, name in enumerate(ordered[:8]):
            ...
            command = "%s %s" % (
                runner, subprocess.list2cmdline([str(name)]))
    ...
    script_names = (
        "start.ps1", "dev.ps1", "run.ps1", "start.cmd", "dev.cmd",
        "run.cmd", "start.bat", "dev.bat", "run.bat")
    for script_name in script_names:
        if os.path.isfile(os.path.join(root, script_name)):
            note_file(script_name)
            command = command_for_script(os.path.join(root, script_name))
            add(command,
    ...
    if not candidates and os.path.isfile(os.path.join(root, "index.html")):
        ...
        add("%s -m http.server 8000" % python_command,
...
def signal_app_stop(target, sig=signal.SIGTERM):
    ...
    return kill_process(ident, force=sig != signal.SIGTERM)
...
def fetch_favicon(port, host="127.0.0.1"):
    ...
    先解析首页 <link rel=...icon...>（含 touch-icon），兜底 /favicon.ico。"""
    ...
    if m:
        add("pip-missing", "缺少 Python 包：%s" % m.group(1),
            "日志报 ModuleNotFoundError: No module named '%s'。" % m.group(1),
                "建议在项目目录创建虚拟环境再安装：py -3 -m venv .venv；.venv\\Scripts\\pip.exe install %s" % m.group(1))
...
def find_northstar_instances():
    ...
    for pid in candidates:
        cwd = cwds.get(pid)
        args = snap.get(pid, {}).get("args") or ""
        server_path = os.path.normcase(os.path.abspath(__file__))
        try:
            same_cwd = (cwd and os.path.realpath(cwd)
                        == os.path.realpath(BASE_DIR))
        except OSError:
            same_cwd = False
        same_dir = bool(same_cwd or server_path in os.path.normcase(args))
        if same_dir and not cwd:
            cwd = BASE_DIR
        if not same_dir:
            continue
        info = snap.get(pid, {})
