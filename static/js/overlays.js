...
function fallbackScriptCommand(path) {
  ...
  if (suffix === '.py') return 'py -3 -- ' + quoted;
  if (suffix === '.ps1') return 'powershell.exe -NoProfile -ExecutionPolicy Bypass -File ' + quoted;
  if (suffix === '.cmd' || suffix === '.bat') return 'cmd.exe /D /S /C ' + quoted;
  if (suffix === '.js') return 'node ' + quoted;
  return quoted;
}
